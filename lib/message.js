 ...
 */
var wechatCore = require('wecwechat-core');
var sms = require('./sms');
    ...
    Booking.findOne({
      ...
      if(!err && booking){
        ...
        if(booking.userId.mobile){
          sms.sendSMS(booking.userId.mobile, '您的定单:' + bookingId + ' 支付成功, 登录海龟租船查看详情');
        }
        ...
        if(booking.ownerId.mobile){
          sms.sendSMS(booking.ownerId.mobile, '客户:' + booking.userId.nickname + ' 预定了您的产品:' + productMsg + '登录海龟租船查看详情');
        }
