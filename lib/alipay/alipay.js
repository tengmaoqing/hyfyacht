...
exports.notify = function(req, res, next){
  alipayCore.verifyNotify(req, function(result){
    if(result.toLocaleLowerCase() == 'true'){
      ...
      var tradeNo = req.body.trade_no;
      var status = req.body.trade_status;
...
exports.return = function(req, res, next){
  alipayCore.verifyReturn(req, function(result){
    if(result.toLocaleLowerCase() == 'true'){
      if(req.body.trade_status == 'TRADE_FINISHED' || req.body.trade_status == 'TRADE_SUCCESS') {
        return res.render('payment/result', {result: 'success'});
      }
    }

    return res.render('payment/result', {result: 'fail'});
  });
};