...
function getAppAccessToken(){
  request.get(wechatConfig.access_token_url, function(err, res, body){
    ...
    if(data.access_token){
      ...
      request.get(wechatConfig.jsapi_ticket_url + data.access_token, function(err, res, body){
        var ticketData = JSON.parse(body);
        if(ticketData.ticket){
          cache.put('wechatJSApiTicket', ticketData.ticket);
        }else{
          console.log(ticketData.errmsg);
        }
      });
    }else{
...
function getParamsForUnifiedorder(ip, wechatOpenId, paramsData, type){
  if(type == 'boat') {
    return {
...
var wechat_core = {
  ...
  unifiedorder: function(ip, wechatOpenId, paramsData, type, callback){
    var params = getParamsForUnifiedorder(ip, wechatOpenId, paramsData, type);
  ...
  getConfigForFrontPage: function(url){
    var params = {
      jsapi_ticket: cache.get('wechatJSApiTicket'),
      timestamp: ((new Date()).getTime() / 1000).toFixed(0),
      noncestr: randomString({length: 16}),
      url:url
    };

    var sign = getJSApiSign(params);
